...
const ProductManagement = () => {
  ...
  const [newProduct, setNewProduct] = useState({ 
    name: '', 
    price: '',  // Base price
    pts: '',    // Price to Stockist (auto-calculated)
    ptr: '',    // Price to Retailer (auto-calculated)
    gst: '',    // GST percentage
  ...
  const calculatePrices = (basePrice, gst, discount) => {
    if (!basePrice || isNaN(basePrice)) return { pts: '', ptr: '', gstAmount: '' };
  ...
  const handlePriceChange = (price) => {
    const { pts, ptr, gstAmount } = calculatePrices(price, newProduct.gst, newProduct.discount);
    setNewProduct({
      ...newProduct,
      price,
      pts,
  ...
  const handleAddProduct = async () => {
    try {
      ...
      setNewProduct({ name: '', price: '', pts: '', ptr: '', gst: '', discount: '', imageUrl: null });
      fetchProducts();
